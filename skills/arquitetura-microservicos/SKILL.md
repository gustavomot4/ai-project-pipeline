...
`CONTEXT.md` + `DECISIONS.md` (o que já foi decidido/rejeitado) + o domínio.
...
**Reprovado se:** um time só, ou sem observabilidade distribuída, ou sem automação de deploy. Nesse caso a entrega é: D-NN REJEITADO + monólito modular ([[skills/arquitetura-monolito/SKILL|arquitetura-monolito]]) com as fronteiras internas desenhadas para permitir extração futura. Diga isso com clareza e sem rodeio.
...
4. **Prefira assíncrono; síncrono é acoplamento de disponibilidade.** Chamada bloqueante só quando o chamador precisa da resposta agora — e então valem as regras de [[skills/microservice-sync/SKILL|microservice-sync]].
