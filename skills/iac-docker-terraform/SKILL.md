...
`CONTEXT.md` (restrições de ambiente e de custo) + o que a aplicação precisa para rodar.
...
7. **Healthcheck que verifica de verdade** (o endpoint responde e a dependência essencial está acessível), não apenas "o processo está de pé".
...
10. **Custo declarado antes do `apply`:** o que este recurso passa a custar por mês. Restrição de custo do `CONTEXT.md` é contrato.
