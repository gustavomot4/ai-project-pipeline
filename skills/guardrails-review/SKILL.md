...
`CONTEXT.md` (as restrições são o contrato a verificar) + o código sob ataque. Ler o `PLANO.md` só para saber o que foi prometido.
...
5. **Segurança:** segredo versionado (`git grep` + histórico), rota/endpoint sensível sem verificação no servidor, autorização só no cliente, injeção, sessão forjável/sem expiração, ausência de limite de tentativas, enumeração de usuário, dado sensível em log.
...
- Restrição de projeto cumprida não é defeito (o `CONTEXT.md` manda; não invente requisito).
- **Não conserte.** Conserto é outra sessão, com autorização do dono.
...
- [ ] Crítico e alto listados como bloqueantes de entrega, com `QA-NN` atribuído em [[DECISIONS]].
