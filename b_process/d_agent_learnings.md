...
- **Regra que a máquina não cobra é pedido, não regra:** "escopo é o módulo desta sessão" viveu como prosa no prompt até virar hook. Antes de escrever mais uma regra, pergunte quem a cobra — se a resposta for "a boa vontade do agente", ela já está escrita e não está valendo.
- **Trava nova nasce com a saída pronta e falhando aberta:** hook que bloqueia errado ensina a desligar o hook, e hook desligado é pior que hook ausente. Toda liberação escreve o motivo, senão não se audita.
- **Placeholder de template não pode virar dado:** um `·` dentro do texto de exemplo partiu o placeholder ao meio e metade dele virou "caminho declarado" — o que faria a trava bloquear tudo. Cheque a linha INTEIRA por `<`, `>` e `…` antes de separar por qualquer coisa.
- **Skill que ninguém alcança é peso morto vestido de cobertura:** o kit tinha uma checagem que se declarava exigida por uma skill que nunca rodou. O problema raramente é falta de skill; é falta de roteamento até ela.
- **Custo que você não consegue medir é custo que você não consegue cobrar:** foi o argumento que matou a ideia de MCP no kit. Todo teto daqui existe porque um script reprova o commit — importar um gasto que mora fora do repositório é importar um teto sem portão.
- **Duplicata não vira verdade por ganhar um guarda:** ao achar o mesmo número em dois arquivos, o reflexo é escrever um teste que compara os dois. O certo é matar a cópia — um lê do outro. Teste vigiando duplicata é a fonte única sendo violada com escolta.
- **Número que você não consegue derivar, date:** "385 testes no projeto medido" não dá para manter fresco, mas "no projeto medido em 2026-08-13" não vira mentira nunca. Número ancorado em data é registro; número solto é dívida.
- **Ressalva não conserta dado sem evidência:** um número que o próprio texto pede para você não usar não está sendo honesto — está cobrando de quem lê a disciplina que ele mesmo não teve. Lacuna declarada é não publicar o número.
- **Prazo único achata a escala de gravidade:** se CRÍTICO e MÉDIO vencem no mesmo dia, a gravidade parou de comprar alguma coisa. E o nível mais baixo não deve vencer nunca — aviso que cobra o que ninguém vai fazer vira ruído, e ruído deixa de ser lido.
- **Separador dentro do item corrompe a contagem do item:** uma lista contada por `·` ganhou dois itens fantasma quando um deles trouxe `·` dentro do próprio parêntese.
