...
- [ ] Decisão fechada → **D-NN** em [[DECISIONS]] (com gatilho, se for estrutural).
- [ ] Bug encontrado → **QA-NN** em [[DECISIONS]] + reprodução em `dev/qa-{{date:YYYY-MM-DD}}.md`.
- [ ] Pendência que é **sua** (não do agente) → **Q-NN** em [[DECISIONS]].
- [ ] "Estado atual" do [[CONTEXT]] reescrito **por substituição** — versão, pronto, em andamento (máx 1), próximo, bloqueado, Q-NN abertas.
- [ ] Nenhum número vigente duplicado fora do [[CONTEXT]] (regra 6).
- [ ] Linha datada no [[CHANGELOG]].
- [ ] [[BACKLOG]] atualizado: card movido, novo card com **portão escrito**.
- [ ] Lição nova? 1 linha em [[APRENDIZADOS]] — inclusive erro do agente.
